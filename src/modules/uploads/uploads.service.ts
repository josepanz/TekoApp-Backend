...
export class UploadsService {
  ...
  async createThumbnail(filename: string, width = 200, height = 200): Promise<string> {
    try {
      const filePath = join(this.uploadPath, filename);
  ...
  async validateFile(file: Express.Multer.File): Promise<boolean> {
    ...
    if (file.size > maxSize) {
      throw new BadRequestException(`El archivo excede el tamaño máximo de ${maxSize / (1024 * 1024)}MB`);
    }
