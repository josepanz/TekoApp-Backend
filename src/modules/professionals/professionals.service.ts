...
export class ProfessionalsService {
  ...
  async registerProfessional(registerDto: any, userId: string): Promise<Professional> {
    ...
    if (registerDto.categoryIds && registerDto.categoryIds.length > 0) {
      const professionalCategories = registerDto.categoryIds.map(categoryId => ({
  ...
  async getProfessionals(filters: ProfessionalFilters): Promise<{ professionals: Professional[]; total: number }> {
    ...
    if (filters.categoryId) {
      queryBuilder.andWhere('categories.categoryId = :categoryId', { categoryId: filters.categoryId });
    }
    ...
    if (filters.minRating) {
      queryBuilder.andWhere('professional.averageRating >= :minRating', { minRating: filters.minRating });
    }

    if (filters.maxPrice) {
      queryBuilder.andWhere('professional.hourlyRate <= :maxPrice', { maxPrice: filters.maxPrice });
    }

    if (filters.isAvailable !== undefined) {
      queryBuilder.andWhere('professional.isAvailable = :isAvailable', { isAvailable: filters.isAvailable });
    }

    // Solo profesionales activos y verificados
    queryBuilder.andWhere('professional.isActive = :isActive', { isActive: true });
  ...
  async getNearbyProfessionals(
    ...
    if (categoryId) {
      queryBuilder.andWhere('categories.categoryId = :categoryId', { categoryId });
    }
  ...
  async updateProfessional(id: string, updateDto: any, userId: string): Promise<Professional> {
    const professional = await this.getProfessionalById(id);

    // Verificar permisos
    if (professional.id !== userId) {
      throw new ForbiddenException('No tienes permisos para modificar este profesional');
    }
  ...
  async updateAvailability(id: string, availabilityDto: any, userId: string): Promise<Professional> {
    const professional = await this.getProfessionalById(id);
  ...
  async updateLocation(id: string, locationDto: any, userId: string): Promise<Professional> {
    const professional = await this.getProfessionalById(id);
  ...
  async addCategories(id: string, categoryIds: string[], userId: string): Promise<Professional> {
    const professional = await this.getProfessionalById(id);
  ...
  async removeCategory(id: string, categoryId: string, userId: string): Promise<Professional> {
    const professional = await this.getProfessionalById(id);
  ...
  async verifyProfessional(id: string, verificationDto: any, adminId: string): Promise<Professional> {
  ...
  async suspendProfessional(id: string, suspensionDto: any, adminId: string): Promise<Professional> {
    // Aquí deberías verificar que el usuario es admin
    const professional = await this.getProfessionalById(id);
  ...
  async searchBySkills(skills: string[], page: number = 1, limit: number = 10): Promise<{ professionals: Professional[]; total: number }> {
    const queryBuilder = this.professionalRepository
      .createQueryBuilder('professional')
      .leftJoinAndSelect('professional.user', 'user')
      .leftJoinAndSelect('professional.categories', 'categories')
  ...
  async getTopRatedProfessionals(categoryId?: string, limit: number = 10): Promise<Professional[]> {
    const queryBuilder = this.professionalRepository
      .createQueryBuilder('professional')
      .leftJoinAndSelect('professional.user', 'user')
      .leftJoinAndSelect('professional.categories', 'categories')
