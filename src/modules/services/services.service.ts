...
export class ServicesService {
  ...
  async createService(createServiceDto: CreateServiceDto, clientId: string): Promise<Service> {
  ...
  async getServices(filters: ServiceFilters): Promise<{ services: Service[]; total: number }> {
    ...
    if (filters.latitude && filters.longitude && filters.radius) {
      ...
      queryBuilder.andWhere(
        'service.latitude BETWEEN :minLat AND :maxLat',
        { minLat: filters.latitude - latRange, maxLat: filters.latitude + latRange }
      );
      queryBuilder.andWhere(
        'service.longitude BETWEEN :minLng AND :maxLng',
        { minLng: filters.longitude - lngRange, maxLng: filters.longitude + lngRange }
      );
    }
  ...
  async updateService(id: string, updateServiceDto: UpdateServiceDto, userId: string): Promise<Service> {
    const service = await this.getServiceById(id);

    // Verificar permisos
    if (service.clientId !== userId && service.professionalId !== userId) {
      throw new ForbiddenException('No tienes permisos para modificar este servicio');
    }

    // Solo permitir actualizaciones en ciertos estados
    if (!service.canBeCancelled()) {
      throw new BadRequestException('No se puede modificar un servicio en este estado');
    }
  ...
  async cancelService(id: string, reason: string, userId: string): Promise<Service> {
    const service = await this.getServiceById(id);

    // Verificar permisos
    if (service.clientId !== userId && service.professionalId !== userId) {
      throw new ForbiddenException('No tienes permisos para cancelar este servicio');
    }

    if (!service.canBeCancelled()) {
      throw new BadRequestException('No se puede cancelar un servicio en este estado');
    }
  ...
  async acceptService(id: string, professionalId: string): Promise<Service> {
    ...
    if (!service.canBeAccepted()) {
      throw new BadRequestException('El servicio no puede ser aceptado en este estado');
    }
  ...
  async startService(id: string, professionalId: string): Promise<Service> {
    ...
    if (service.professionalId !== professionalId) {
      throw new ForbiddenException('Solo el profesional asignado puede iniciar el servicio');
    }

    if (!service.canBeStarted()) {
      throw new BadRequestException('El servicio no puede ser iniciado en este estado');
    }
  ...
  async completeService(id: string, professionalId: string): Promise<Service> {
    ...
    if (service.professionalId !== professionalId) {
      throw new ForbiddenException('Solo el profesional asignado puede completar el servicio');
    }

    if (!service.canBeCompleted()) {
      throw new BadRequestException('El servicio no puede ser completado en este estado');
    }
  ...
  async rateService(id: string, ratingDto: RateServiceDto, userId: string): Promise<Service> {
    const service = await this.getServiceById(id);
    ...
    // Verificar permisos
    if (service.clientId !== userId && service.professionalId !== userId) {
      throw new ForbiddenException('No tienes permisos para calificar este servicio');
    }
  ...
  async createServiceRequest(
    ...
    if (service.status !== ServiceStatus.PENDING) {
      throw new BadRequestException('Solo se pueden crear solicitudes para servicios pendientes');
    }
    ...
    if (existingRequest) {
      throw new BadRequestException('Ya has enviado una solicitud para este servicio');
    }
  ...
  async respondToServiceRequest(
    ...
    if (service.clientId !== userId) {
      throw new ForbiddenException('Solo el cliente puede responder a las solicitudes');
    }
    ...
    if (responseDto.status === RequestStatus.ACCEPTED) {
      ...
      await this.serviceRequestRepository.update(
        ...
        { status: RequestStatus.REJECTED, rejectionReason: 'Otra solicitud fue aceptada' },
      );
  ...
  async getDashboardStats(userId: string): Promise<ServiceStats> {
    ...
    if (isProfessional) {
      servicesQuery = servicesQuery.where('service.professionalId = :userId', { userId });
    } else {
      servicesQuery = servicesQuery.where('service.clientId = :userId', { userId });
    }

    const [total, pending, inProgress, completed, cancelled] = await Promise.all([
      servicesQuery.getCount(),
      servicesQuery.andWhere('service.status = :status', { status: ServiceStatus.PENDING }).getCount(),
      servicesQuery.andWhere('service.status = :status', { status: ServiceStatus.IN_PROGRESS }).getCount(),
      servicesQuery.andWhere('service.status = :status', { status: ServiceStatus.COMPLETED }).getCount(),
      servicesQuery.andWhere('service.status = :status', { status: ServiceStatus.CANCELLED }).getCount(),
    ]);
    ...
    const ratingResult = await this.serviceRepository
      ...
      .where(isProfessional ? 'service.professionalId = :userId' : 'service.clientId = :userId', { userId })
