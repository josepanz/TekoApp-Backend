...
| H-01 | ALTO | [ ] | `5551d5c` (opciones), decisión en el doc | **DECIDIDO 2026-09-07, sin implementar todavía**: José eligió GlitchTip (hosted free, 1.000 eventos/mes, sin tarjeta) + Seq (licencia Individual gratuita, self-hosted, 1 usuario, sin tarjeta). Ver la sección "DECISIÓN DE JOSÉ" de H-01-observability-options.md. Corrige un supuesto de ese doc: Seq tiene el CÓDIGO integrado pero el servidor NO corre en ningún lado (`SEQ_ENABLED=false`, sin entrada en docker-compose ni en `ci/`), así que gratis es de licencia, no de hosting. Plan en dos tiempos: (1) GlitchTip hosted ahora vía `@sentry/nestjs` con DSN por env var, enganchado en `AllExceptionsFilter`, scrubbing reusando `ObservabilityModule.formatPayload` y `beforeSend` con dedup/muestreo por el tope de eventos; (2) Seq en producción cuando haya dónde hostearlo (hoy solo local). NO cerrar H-01 hasta que las dos mitades estén arriba |
