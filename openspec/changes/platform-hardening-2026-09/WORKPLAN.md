...
| H-01 | ALTO | [ ] | `5551d5c` (opciones), `38d4ce1` (GlitchTip) | **Mitad 1 implementada 2026-09-12** (`38d4ce1`): `@sentry/nestjs` apuntando a GlitchTip, DSN por `GLITCHTIP_DSN` (Joi, opcional — sin DSN el SDK no inicializa y la app arranca igual, verificado con test), enganchado en `AllExceptionsFilter` (el filtro que atrapa lo NO manejado, confirmado leyendo `selectExceptionFilterMetadata` de NestJS) con contexto de request (ruta, método, requestId, userId), scrubbing reusando `ObservabilityModule.formatPayload` (sin sanitizador nuevo) y `beforeSend` con dedup (60s) + muestreo (10%) para el tope de 1.000 eventos/mes. Ver la sección "DECISIÓN DE JOSÉ" de H-01-observability-options.md. **Mitad 2 (Seq en producción) sigue pendiente** — hoy solo corre local (`SEQ_ENABLED=false`). NO cerrar H-01 hasta que las dos mitades estén arriba |
...
| I-02 | ALTO | [ ] | `97afed1`, ver doc | **Bloqueante identificado 2026-09-07**: Dinelco/BEPSA NO hace envíos a terceros (solo a comercios adheridos a su red), así que NO hay split payment y Dinelco queda acotado al cobro. El bloqueo pasó de "qué ofrece Dinelco" a "elegir proveedor de pagos salientes" (candidatos sin evaluar: SIPAP, BCP, otra solución bancaria). Se para acá por decisión de José. Dos gaps nuevos registrados en I-02-payout-open-questions.md: no existe ningún modelo de datos bancarios en el schema, y el contrato de `PaymentMethodEntity.details` está abierto (riesgo PCI — regla nueva en database-conventions.md). Aclarado además que "solo profesional" no existe: un profesional es un Users con perfil profesional |
...
| I-05 | MEDIO | [x] | `e555b72` | Fuera del inventario original de este WORKPLAN — agregada 2026-09-14, a pedido de Mobile (`notification-preferences-and-inbox.md` la deja anotada como gap de backend). Spec en I-05-notification-triggers.md. Verificación previa corrigió el enunciado: hay 1 disparo real (`ProfessionalDocumentsExpirationJob`, cron), no cero — se usa como ejemplo del patrón a replicar, no invalida el hallazgo. 34 filas de inventario (16 IMPRESCINDIBLE, 15 DESEABLE, 1 IMPRESCINDIBLE bloqueada por I-02/`PAYMENT_RECEIVED`). Hallazgos colaterales: `verifyProfessional`/`suspendProfessional` alineados al patrón `updateMany`+condicional en la tarea 2 de la tanda 2026-09-14 (commit `80fa823`); canal `email` del `NotificationsProcessor` cableado a `EmailService` en la tarea 3 de esa misma tanda (commit `1ab7f42`) — canal `sms` sigue como stub a propósito (sin paquete `twilio` ni cliente en el repo, solo env vars validadas). Módulo de preferencias de notificación (tarea 4, commit `41fc3a8`, migración `20260914120000_add_notification_preferences` aplicada). **Los 18 disparos IMPRESCINDIBLE implementados (tarea 5, 2026-09-14)**, 8 commits por dominio: `b3e34b2` servicios, `da7f36d` presupuestos, `f7d94b5` contratos, `4d18513` pagos, `a6cd854` documentos profesionales, `b99cd02` disputas, `45eb27e` borrado de cuenta, `aac052d` profesionales. Corrección de conteo: la tabla marca 18 filas IMPRESCINDIBLE, no 16 — ver `decisions.md`. `PAYMENT_RECEIVED` (#15) sigue sin implementar, bloqueado por I-02. Ver `decisions.md` para el detalle completo |
| I-06 | MEDIO | [x] | `5892ac5` | Fuera del inventario original de este WORKPLAN — agregada 2026-09-14, a pedido explícito de José, retomando `TekoApp-Frontend-Mobile/openspec/specs/support-channel.md` (parte cliente ya diseñada, bloqueada esperando esta definición de backend). Spec en I-06-support-channel.md: modelo `SupportRequests` + 3 endpoints (`POST /support/contact`, `GET /support/me`, `GET /admin/support-requests`) + permiso nuevo `SUPPORT.AUDIT_VIEW` + reutiliza `EmailModule` (con una advertencia anotada: `sendEmailByType` asume un `Users` real como destinatario, la casilla de soporte no lo es) + recomienda sumar la ruta al limitador `upload` ya cableado en D-02 en vez de crear uno nuevo. Solo spec — cero cambios en `src/` |
...
| T-03 | BAJO | [x] | `53de172`, `e3b466a` | Typo de archivo corregido en `53de172`. Rename de módulos completado en `e3b466a` (2026-09-14, deuda diferida ejecutada a pedido de José): `locations-db`→`professional-position-db`, `tracking-db`→`geo-tracking-db`, con docstring en cada módulo explicando la división. Detalle en `openspec/decisions.md` |
...
|---|---|---|---|
| `I-01` de Web (`TekoApp-Frontend-Web`) | 3 endpoints admin que Web necesita | `openspec/changes/0015-admin-backoffice-endpoints.md` (`W-01`..`W-03`) | **[x] las 3 implementadas** — `220b801` (W-01 audit-logs), `14c80d6`+`cf63580` (W-02 export CSV pagos y profesionales, incluyó un fix real de `TransformInterceptor` que rompía descargas binarias), `81083a8` (W-03 búsqueda por texto). Archivo cerrado en `ff38236`. Web ya regeneró sus tipos (`750e0d4` en su repo) — le queda implementar las 3 pantallas |
| `I-01` de este WORKPLAN | Implementar el borrado de cuenta especificado | `platform-hardening-2026-09/I-01-account-deletion.md` | **[x] implementado** — `a24cf7f`, migración aplicada. **El bloqueante de disputas quedó cerrado en `262c071`** (I-03): `AccountDeletionService.findBlockers` ahora cuenta disputas `OPEN`/`UNDER_REVIEW` del usuario (`DeletionBlockerType.OPEN_DISPUTE`) |
...
| hallazgo de paso en `I-03` | 5 endpoints admin sin permiso real (`@Permissions` de clase que el guard nunca lee) | ver abajo | **[x] corregido** — `beb7e16` (parche puntual), **[x] arreglo de fondo del guard** — `07c852e` (2026-09-12) |
