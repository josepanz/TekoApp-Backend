...
| I-05 | MEDIO | [x] | `e555b72` | Fuera del inventario original de este WORKPLAN — agregada 2026-09-14, a pedido de Mobile (`notification-preferences-and-inbox.md` la deja anotada como gap de backend). Spec en I-05-notification-triggers.md. Verificación previa corrigió el enunciado: hay 1 disparo real (`ProfessionalDocumentsExpirationJob`, cron), no cero — se usa como ejemplo del patrón a replicar, no invalida el hallazgo. 34 filas de inventario (16 IMPRESCINDIBLE, 15 DESEABLE, 1 IMPRESCINDIBLE bloqueada por I-02/`PAYMENT_RECEIVED`). Hallazgos colaterales anotados (no corregidos): canales `email`/`sms` del `NotificationsProcessor` son stubs sin wiring real; `verifyProfessional`/`suspendProfessional` no usan el patrón `updateMany`+condicional del resto del repo |
